...
public class AppController {
    ...
    private static final String MESSAGE_BUNDLE = "com.sportt5.i18n.messages";
    private static final Preferences PREFS = Preferences.userNodeForPackage(AppController.class);
    private static final Map<String, String> EN_TRANSLATIONS = loadTranslations(ENGLISH);
    private static final Map<String, String> VI_TRANSLATIONS = loadTranslations(VIETNAMESE);
    private static final Map<String, String> VI_TO_EN = reverseTranslations(VI_TRANSLATIONS);
    ...
    private void signIn() {
        if (isBlank(loginEmailField.getText()) || isBlank(loginPasswordField.getText())) {
            setStatusText(loginStatusLabel, "Enter your email and password.");
            return;
        }
        ...
        setStatusText(loginStatusLabel, "Use user, artist, or admin mock account.");
    }

    @FXML
    private void signUp() {
        if (isBlank(signupNameField.getText()) || isBlank(signupEmailField.getText()) || isBlank(signupPasswordField.getText())) {
            setStatusText(signupStatusLabel, "Complete all fields to continue.");
            return;
        }
    ...
    private void forgotPassword() {
        setStatusText(loginStatusLabel, "Password reset is mocked in this frontend build.");
    }
    ...
    private void setStatusText(Label label, String englishText) {
        label.setText(translate(englishText));
    }
    ...
    private String translate(String value) {
        ...
        return VIETNAMESE.equals(currentLanguage)
                ? VI_TRANSLATIONS.getOrDefault(english, english)
                : EN_TRANSLATIONS.getOrDefault(english, english);
    }

    private static Map<String, String> reverseTranslations(Map<String, String> translations) {
    ...
    private static Map<String, String> loadTranslations(String language) {
        try {
            ResourceBundle bundle = ResourceBundle.getBundle(MESSAGE_BUNDLE, Locale.forLanguageTag(language));
